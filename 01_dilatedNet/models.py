...
class RDB(nn.Module):
    def __init__(self, inChannel, growthRate, layerDepth, dilation):
...
class RDN(nn.Module):
    ...
    def __init__(self, colorChannel, scale, blockDepth, convDepth ):
        ...
        #RDBs
        layers = []
        for i in range(int(self.blockDepth/4)):
            layers.append(RDB(self.g0, self.growthRate, self.convDepth, 4))
        for i in range(int(self.blockDepth/4)):
            layers.append(RDB(self.g0, self.growthRate, self.convDepth, 2))
        for i in range(int(self.blockDepth/2)):
            layers.append(RDB(self.g0, self.growthRate, self.convDepth, 1))
